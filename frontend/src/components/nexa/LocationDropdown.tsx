  ...
  return (
    <div className={cn("relative", className)} ref={dropdownRef}>
      <motion.button
        ...
        className={cn(
          "flex items-center gap-2 px-4 py-2.5 liquid-glass rounded-full hover:border-nexa-brand transition-all border border-slate-200/40 text-slate-800 shadow-sm",
          buttonClassName
        )}
      >
        <MapPin className="w-4 h-4 text-nexa-brand animate-pulse" />
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full mt-2 left-0 w-72 bg-nexa-bg-base border border-nexa-border rounded-2xl shadow-2xl z-[500] overflow-hidden"
          >
            ...
            <div className="max-h-80 overflow-y-auto p-2 no-scrollbar">
              <button
                ...
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-nexa-brand/10 text-nexa-brand transition-colors text-left mb-1 disabled:opacity-60"
              >
                {isDetecting ? (
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelectedCity
                        ? "bg-nexa-brand/5 border-nexa-brand/20 text-nexa-brand"
                        : "bg-transparent border-transparent hover:bg-nexa-bg-surface text-nexa-text-primary"
                    )}
                  >
                    <span>{city.name}</span>
                    {isSelectedCity && !currentArea && (
                      <Check className="w-4 h-4 text-nexa-brand" />
                    )}
                  </button>
                );
              })}
