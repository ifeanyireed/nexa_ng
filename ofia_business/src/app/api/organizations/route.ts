...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch organizations: " + errText }, { status: res.status });
  } catch (err: any) {
    return NextResponse.json(
      { error: "Failed to fetch organizations from backend database: " + err.message },
      { status: 502 }
    );
  }
}
