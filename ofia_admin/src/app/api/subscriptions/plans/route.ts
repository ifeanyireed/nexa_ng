...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch plans from backend database: " + err.message }, { status: 502 });
  }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "Failed to persist plan to database: " + err.message }, { status: 502 });
  }
}
