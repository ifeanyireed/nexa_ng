...
export default function AgedReceivablesPage() {
	...
	const [receivables] = useState<AgedReceivableRow[]>([
		{ customer: "Ecologique Transport Solution", totalDue: 6000.00, current: 6000.00, days1_30: 0, days31_60: 0, days61_90: 0, days90_plus: 0 },
		{ customer: "IHS - Holding Limited", totalDue: 1000000.00, current: 0, days1_30: 1000000.00, days31_60: 0, days61_90: 0, days90_plus: 0 },
		{ customer: "7UP Bottling Company", totalDue: 106375.00, current: 6375.00, days1_30: 0, days31_60: 100000.00, days61_90: 0, days90_plus: 0 }
	]);
	...
	return (
		<div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100/40 flex flex-col gap-6 animate-fadeIn text-[#1e293b]">
			...
			<div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
				<div>
					<h2 className="text-[20px] font-black text-slate-800 tracking-tight">Aged Receivables</h2>
					<p className="text-xs text-slate-455 font-semibold mt-1">Review aged customer outstanding invoices and overdue payments</p>
				</div>
				<button
					onClick={() => alert("Exporting aged receivables summary...")}
				...
				<table className="w-full text-left border-collapse select-none">
					<thead>
						<tr className="border-b border-gray-100 text-slate-455 text-[11px] font-bold uppercase tracking-wider">
							<th className="pb-3 min-w-[200px]">Customer</th>
					...
					<tbody className="divide-y divide-gray-50 text-xs font-semibold text-slate-700">
						{filtered.map((r, idx) => (
							<tr key={idx} className="hover:bg-slate-50/50 transition-colors">
								<td className="py-4 text-slate-800 font-bold">{r.customer}</td>
								<td className="py-4 text-slate-850 font-black">{formatNaira(r.totalDue)}</td>
								<td className="py-4 text-emerald-650">{formatNaira(r.current)}</td>
								<td className="py-4 text-slate-600">{formatNaira(r.days1_30)}</td>
								<td className="py-4 text-slate-600">{formatNaira(r.days31_60)}</td>
								<td className="py-4 text-slate-600">{formatNaira(r.days61_90)}</td>
								<td className="py-4 text-red-500">{formatNaira(r.days90_plus)}</td>
							</tr>
						))}
						{/* Summary Row */}
