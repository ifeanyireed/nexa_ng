"use client";

import React, { useState } from "react";
import { useRouter } from "next/navigation";
import { 
	IconPlus, 
	IconSearch,
	IconTrash 
} from "@tabler/icons-react";

const TRANSACTIONS_TABS = [
	{ id: "journal-entries", label: "Journal Entries", slug: "/accountant/journal-entries" },
	{ id: "recurring-journals", label: "Recurring Journals", slug: "/accountant/recurring-journals" },
	{ id: "reconcile", label: "Bank Reconciliation", slug: "/accountant/reconcile" },
	{ id: "budget-planner", label: "Budget Planner", slug: "/accountant/budget-planner" },
	{ id: "period-close", label: "Period Close", slug: "/accountant/period-close" },
	{ id: "audit-trail", label: "Audit Trail", slug: "/accountant/audit-trail" }
];

interface RecurringJournal {
	id: string;
	name: string;
	interval: string;
	lastRun: string;
	nextRun: string;
	amount: number;
	status: string;
}

export default function RecurringJournalsPage() {
	const router = useRouter();

	const [templates, setTemplates] = useState<RecurringJournal[]>([
		{ id: "RJ-00001", name: "Monthly Rent Accrual", interval: "Monthly", lastRun: "01-07-2026", nextRun: "01-08-2026", amount: 1200000.00, status: "Active" },
		{ id: "RJ-00002", name: "Weekly Logistics Accrual", interval: "Weekly", lastRun: "15-07-2026", nextRun: "22-07-2026", amount: 450000.00, status: "Active" }
	]);

	const [searchQuery, setSearchQuery] = useState("");
	const [showAddModal, setShowAddModal] = useState(false);

	const [newTemplate, setNewTemplate] = useState({
		name: "",
		interval: "Monthly",
		amount: "",
		nextRun: ""
	});

	const filteredTemplates = templates.filter(t =>
		t.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
		t.id.toLowerCase().includes(searchQuery.toLowerCase())
	);

	const handleAddTemplateSubmit = (e: React.FormEvent) => {
		e.preventDefault();
		const amountVal = parseFloat(newTemplate.amount);
		if (isNaN(amountVal) || amountVal <= 0) {
			alert("Please enter a valid amount");
			return;
		}

		const padNum = String(templates.length + 1).padStart(5, "0");
		const templateData: RecurringJournal = {
			id: `RJ-${padNum}`,
			name: newTemplate.name,
			interval: newTemplate.interval,
			lastRun: "--",
			nextRun: newTemplate.nextRun.split("-").reverse().join("-") || "01-08-2026",
			amount: amountVal,
			status: "Active"
		};

		setTemplates(prev => [...prev, templateData]);
		setShowAddModal(false);
		setNewTemplate({ name: "", interval: "Monthly", amount: "", nextRun: "" });
	};

	const handleDeleteTemplate = (id: string) => {
		if (confirm(`Are you sure you want to delete recurring journal template ${id}?`)) {
			setTemplates(prev => prev.filter(t => t.id !== id));
		}
	};

	const formatNaira = (amount: number) => {
		return new Intl.NumberFormat("en-NG", {
			style: "currency",
			currency: "NGN",
			minimumFractionDigits: 2
		}).format(amount);
	};

	return (
		<div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100/40 flex flex-col gap-6 animate-fadeIn text-[#1e293b]">
			
			{/* Header */}
			<div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
				<div>
					<h2 className="text-[20px] font-black text-slate-800 tracking-tight">Recurring Journals</h2>
					<p className="text-xs text-slate-455 font-semibold mt-1">Manage scheduled journal templates for automated period adjustments</p>
				</div>
				<button
					onClick={() => setShowAddModal(true)}
					className="px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white font-extrabold rounded-xl text-xs flex items-center gap-1.5 cursor-pointer shadow-sm active:scale-95 transition-all border-none outline-none self-start sm:self-center"
				>
					<IconPlus className="w-4 h-4" />
					Add Recurring Journal
				</button>
			</div>

			{/* Sub-menu Tabs */}
			<div className="flex border-b border-gray-200 overflow-x-auto scrollbar-hide gap-1">
				{TRANSACTIONS_TABS.map((tab) => (
					<button
						key={tab.id}
						onClick={() => router.push(tab.slug)}
						className={`px-5 py-3 font-extrabold text-xs whitespace-nowrap border-b-2 transition-all cursor-pointer ${
							tab.id === "recurring-journals"
								? "border-red-500 text-red-500 font-bold"
								: "border-transparent text-slate-400 hover:text-slate-700"
						}`}
					>
						{tab.label}
					</button>
				))}
			</div>

			{/* Search */}
			<div className="flex justify-end">
				<div className="relative w-full sm:w-64">
					<input
						type="text"
						placeholder="Search templates..."
						value={searchQuery}
						onChange={(e) => setSearchQuery(e.target.value)}
						className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 font-semibold outline-none"
					/>
					<IconSearch className="absolute left-3 top-2.5 w-4 h-4 text-slate-400" />
				</div>
			</div>

			{/* Table Grid */}
			<div className="overflow-x-auto min-h-60">
				<table className="w-full text-left border-collapse select-none">
					<thead>
						<tr className="border-b border-gray-100 text-slate-455 text-[11px] font-bold uppercase tracking-wider">
							<th className="pb-3 min-w-[120px]">Template ID</th>
							<th className="pb-3 min-w-[200px]">Template Name</th>
							<th className="pb-3 min-w-[110px]">Interval</th>
							<th className="pb-3 min-w-[110px]">Last Run</th>
							<th className="pb-3 min-w-[110px]">Next Run</th>
							<th className="pb-3 min-w-[140px]">Total Amount</th>
							<th className="pb-3 min-w-[100px]">Status</th>
							<th className="pb-3 text-right w-20">Action</th>
						</tr>
					</thead>
					<tbody className="divide-y divide-gray-50">
						{filteredTemplates.length > 0 ? (
							filteredTemplates.map((t) => (
								<tr key={t.id} className="hover:bg-slate-50/50 transition-colors text-xs font-semibold text-slate-700">
									<td className="py-4 font-mono font-bold text-slate-800">{t.id}</td>
									<td className="py-4 text-slate-800">{t.name}</td>
									<td className="py-4 text-slate-550">{t.interval}</td>
									<td className="py-4 text-slate-550">{t.lastRun}</td>
									<td className="py-4 text-slate-550">{t.nextRun}</td>
									<td className="py-4 text-slate-800 font-bold">{formatNaira(t.amount)}</td>
									<td className="py-4">
										<span className="bg-emerald-50 text-emerald-700 border border-emerald-100 font-extrabold text-[9px] px-2 py-0.5 rounded-md uppercase">
											{t.status}
										</span>
									</td>
									<td className="py-4 text-right">
										<button
											onClick={() => handleDeleteTemplate(t.id)}
											className="p-1 hover:bg-red-50 text-red-500 hover:text-red-700 rounded-lg cursor-pointer transition-colors border-none bg-transparent"
										>
											<IconTrash className="w-4 h-4" />
										</button>
									</td>
								</tr>
							))
						) : (
							<tr>
								<td colSpan={8} className="py-12 text-center text-xs text-slate-400 font-bold italic">
									No templates found matching filters.
								</td>
							</tr>
						)}
					</tbody>
				</table>
			</div>

			{/* ADD TEMPLATE MODAL */}
			{showAddModal && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-fadeIn">
					<div className="bg-white rounded-3xl w-full max-w-md shadow-2xl border border-gray-100 flex flex-col overflow-hidden animate-slideUp">
						<div className="flex justify-between items-center p-6 border-b border-gray-100 bg-slate-50/50">
							<div>
								<h3 className="font-extrabold text-slate-800 text-sm">Add Recurring Journal</h3>
								<p className="text-[10px] text-slate-455 mt-0.5 font-semibold">Define a scheduled automated journal entry template</p>
							</div>
							<button
								onClick={() => setShowAddModal(false)}
								className="w-7 h-7 rounded-xl hover:bg-slate-100 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors cursor-pointer text-sm font-bold border-none bg-transparent"
							>
								✕
							</button>
						</div>

						<form onSubmit={handleAddTemplateSubmit} className="p-6 flex flex-col gap-4 text-left">
							<div className="flex flex-col gap-1">
								<label className="text-[10px] font-bold text-slate-450 uppercase">Template Name</label>
								<input
									type="text"
									required
									placeholder="e.g. Monthly Accrual Adjustments"
									value={newTemplate.name}
									onChange={(e) => setNewTemplate(prev => ({ ...prev, name: e.target.value }))}
									className="px-3 py-2 bg-gray-50 border border-gray-200 text-xs rounded-xl focus:outline-none focus:border-blue-500 text-slate-800 font-semibold"
								/>
							</div>

							<div className="grid grid-cols-2 gap-3">
								<div className="flex flex-col gap-1">
									<label className="text-[10px] font-bold text-slate-450 uppercase">Frequency / Interval</label>
									<select
										value={newTemplate.interval}
										onChange={(e) => setNewTemplate(prev => ({ ...prev, interval: e.target.value }))}
										className="px-3 py-2 bg-gray-50 border border-gray-200 text-xs rounded-xl focus:outline-none focus:border-blue-500 text-slate-800 font-semibold"
									>
										<option value="Weekly">Weekly</option>
										<option value="Monthly">Monthly</option>
										<option value="Quarterly">Quarterly</option>
										<option value="Yearly">Yearly</option>
									</select>
								</div>
								<div className="flex flex-col gap-1">
									<label className="text-[10px] font-bold text-slate-455 uppercase">Start Date</label>
									<input
										type="date"
										required
										value={newTemplate.nextRun}
										onChange={(e) => setNewTemplate(prev => ({ ...prev, nextRun: e.target.value }))}
										className="px-3 py-2 bg-gray-50 border border-gray-200 text-xs rounded-xl focus:outline-none focus:border-blue-500 text-slate-800 font-semibold"
									/>
								</div>
							</div>

							<div className="flex flex-col gap-1">
								<label className="text-[10px] font-bold text-slate-455 uppercase">Total Transaction Amount (NGN)</label>
								<input
									type="number"
									required
									placeholder="e.g. 1200000"
									value={newTemplate.amount}
									onChange={(e) => setNewTemplate(prev => ({ ...prev, amount: e.target.value }))}
									className="px-3 py-2 bg-gray-50 border border-gray-200 text-xs rounded-xl focus:outline-none focus:border-blue-500 text-slate-800 font-semibold"
								/>
							</div>

							<button
								type="submit"
								className="w-full py-2.5 mt-2 bg-red-500 hover:bg-red-600 text-white font-extrabold rounded-xl text-xs transition-all shadow-md active:scale-[0.99] border-none outline-none cursor-pointer"
							>
								Create Template
							</button>
						</form>
					</div>
				</div>
			)}

		</div>
	);
}
