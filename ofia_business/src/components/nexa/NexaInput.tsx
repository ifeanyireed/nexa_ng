...
export interface NexaInputProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, "prefix"> {
  label?: string;
...
export const NexaInput = React.forwardRef<HTMLInputElement, NexaInputProps>(
  ({ className, label, variant = "default", error, helperText, prefix, leftIcon, rightIcon, onFocus, onBlur, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-1 w-full">
        <div
          className={cn(
            "relative flex items-center h-12 rounded-lg border-[0.5px] bg-nexa-bg-surface transition-all duration-200",
        ...
        >
          {variant === "search" && (
            <div className="pl-3 pr-2 text-nexa-text-faint">
              <Search className="w-5 h-5" />
            </div>
          )}

          {variant === "phone" && (
            <div className="flex items-center gap-2 px-3 border-r border-nexa-border text-nexa-text-primary">
          ...
          <AnimatePresence>
            {variant === "search" && hasValue && (
              <motion.button
                ...
                className="pr-3 text-nexa-text-faint hover:text-nexa-text-secondary"
              >
                <X className="w-5 h-5" />
              </motion.button>
            )}
        ...
        {helperText && !error && (
          <span className="text-[11px] text-[var(--nexa-text-muted)] px-1">{helperText}</span>
        )}
      </div>
    );
  }
